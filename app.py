...
@app.route('/resultwoman.html', methods = ['POST'])
def predict2():
    lrm = joblib.load('regmodel')
    print('model loaded')
    ...
    if request.method == 'POST':
        ...
        my_prediction = lrm.predict(X)

    return render_template('resultwoman.html',year=year, district = address2,prediction=int(my_prediction))
